 ...
 *  @brief FFunction implimentation for the SD card helpers.
 *
...
bool SD_Detect()
{
  ...
  delay(5);
  hspi->begin(18, 19, 23, 5);               //SCLK, MISO, MOSI, SS
  if (!SD.begin(SS, SPI, 18000000, "/sd"))  //Chip Select , SPI instance, SPI Frequency, SD card mount point
  {
    SDCARDLogLn("Card Mount Failed");
    return false;
  }
...
const char* SD_FirstFileOfDir(const char * dirname, uint8_t levels)
{
  //SDCARDLogF("Listing first file of dir: %s\n", dirname);
